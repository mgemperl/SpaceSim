...
#define MISSILE_DEF ShipDef("boom", MISSILE, ShipStats(100, 100, 100, 100, 0.5, 0, 0, 0.2, 20, 0, 0), PLAYERFACTION)
...
bool WeaponSlot::IsInArc(const Entity* pEntity) const
{
	...
	if (pEntity != NULL)
	{
		isInArc = Vector2D::AngleDiff(m_DefaultFacing + m_pShip->GetOrientationRad(), 
			Vector2D::ComputeAngle(GetPos(), pEntity->ProjectCollisionPointSimple(GetPos(), m_WeapDef.speed))) <= (m_FiringArc / 2.0);
	}
...
void WeaponSlot::FireGuided()
{
	if (m_WeaponInstalled && (m_TicksUntilReload <= 0))
	{
		...
		m_pShip->GetSector()->AddDrawableEntity(proj);

		m_TicksUntilReload = m_WeapDef.reloadTime;
